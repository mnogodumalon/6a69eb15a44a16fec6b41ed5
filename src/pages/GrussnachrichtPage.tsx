...
export default function GrussnachrichtPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Grussnachricht[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Grussnachricht | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getGrussnachricht());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Grussnachricht['fields']) {
  ...
  async function handleUpdate(fields: Grussnachricht['fields']) {
    ...
    await LivingAppsService.updateGrussnachrichtEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteGrussnachrichtEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Grußnachricht"
      subtitle={`${records.length} Grußnachricht im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Grußnachricht suchen..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Grußnachricht. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
