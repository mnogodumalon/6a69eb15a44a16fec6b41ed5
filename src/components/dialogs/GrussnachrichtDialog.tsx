...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Grussnachricht';
...
interface GrussnachrichtDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Grussnachricht['fields']) => Promise<void>;
...
export function GrussnachrichtDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: GrussnachrichtDialogProps) {
  const [fields, setFields] = useState<Partial<Grussnachricht['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Grussnachricht['fields']>);
      setPreview(null);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Grussnachricht['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Grußnachricht bearbeiten' : 'Grußnachricht hinzufügen';
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.GRUSSNACHRICHT} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
